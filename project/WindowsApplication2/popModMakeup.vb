Public Class popModMakeup
    ...
    Private Sub bttnModify_Click(sender As Object, e As EventArgs) Handles bttnModify.Click
        ...
        Else
            ...
            dbAccess.updateData("UPDATE `introse`.`makeup` SET `timestart` = '" & txtbxStart.Text & "', `timeend` = '" & txtbxEnd.Text & "',`room` = '" & txtbxRoom.Text & "', `reason_cd` = '" & reason_cd & "', `makeup_date` = '" & dtpMakeUpDate.Value.Date.ToString("yyyy-MM-dd") & "', `enc_date` = '" & Date.Now.Date.ToString("yyyy-MM-dd") & "', `encoder` =  '" & txtbxEncoder.Text & "' WHERE `makeupid` = '" & wdwFacultyMakeUp.RData(0) & "';")
