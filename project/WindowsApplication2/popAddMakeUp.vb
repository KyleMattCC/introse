    ...
    Private Sub bttnAdd_Click(sender As Object, e As EventArgs) Handles bttnAdd.Click
        ...
        Try
            ...
            Else
                ...
                reason_cd = dbAccess.getData("SELECT reason_cd FROM reason where reason_desc = '" & txtReason.Text & "'", "reason_cd")
                fac = dbAccess.getData("SELECT facref_no FROM faculty where facultyid = '" & txtbxFacID.Text & "'", "facref_no")
                course = cmbbxCourse.SelectedItem.ToString
                course_id = dbAccess.getData("select course_id from course where course_cd = '" & course & "';", "course_id")
                section = cmbbxSec.SelectedItem.ToString
                courseoffering_id = dbAccess.getData("select courseoffering_id from courseoffering where facref_no = '" & fac & "' and course_id = '" & course_id & "' and section = '" & section & "';", "courseoffering_id")
