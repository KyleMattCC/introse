Public Class popEncFacDaily
    ...
    Private Sub AddRoom(facultyid As String, course As String, section As String, ByVal text As TextBox)
        ...
        Try
            ...
            room = dbAccess.getStringData("SELECT co.room FROM courseoffering AS co, course AS c WHERE co.facref_no = '" & fac & "' AND co.status = 'A' AND c.course_cd = '" & course & "' AND co.section = '" & section & "';", "room")
    ...
    Private Sub SetAttendance(facultyid As String, course As String, section As String, remark As String, inputdate As Date, encoder As String, checker As String, ByVal text As TextBox, ByVal combo As ComboBox, stat As String)
        ...
        Try
            ...
            Else
                courseid = dbAccess.getStringData("SELECT course_id FROM course WHERE course_cd ='" & course & "';", "course_id")
                fac = dbAccess.getStringData("select facref_no from faculty where facultyid = '" & facultyid & "';", "facref_no")
                ...
                remark_cd = dbAccess.getStringData("select remark_cd from remarks where remark_des = '" & remark & "';", "remark_cd")
    ...
    Private Sub AddSched(facultyid As String, course As String, section As String, ByVal TimeStart As TextBox, ByVal TimeEnd As TextBox, ByVal DaySched As TextBox)
        ...
        Try
            ...
            starttime = dbAccess.getStringData("SELECT co.timestart FROM courseoffering as co, course as c WHERE c.course_cd = '" & course & "' AND co.facref_no = '" & fac & "' AND co.section = '" & section & "' AND co.status = 'A';", "timestart")
            MsgBox("starttime: " + starttime)
            endtime = dbAccess.getStringData("SELECT co.timeend FROM courseoffering as co, course as c WHERE c.course_cd = '" & course & "' AND co.facref_no = '" & fac & "' AND co.section = '" & section & "' AND co.status = 'A';", "timeend")
            MsgBox("endtime: " + endtime)
            sched = dbAccess.getStringData("SELECT co.daysched FROM courseoffering as co, course as c WHERE c.course_cd = '" & course & "' AND co.facref_no = '" & fac & "' AND co.section = '" & section & "' AND co.status = 'A';", "daysched")
            MsgBox("sched: " + sched)
    ...
    Private Sub cmbbxRemarks_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbbxRemarks.SelectedIndexChanged
        txtbxRemarks.Text = cmbbxRemarks.SelectedItem.ToString
    End Sub
