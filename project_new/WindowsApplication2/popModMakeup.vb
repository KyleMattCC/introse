    ...
    Private Sub bttnModify_Click(sender As Object, e As EventArgs) Handles bttnModify.Click
        ...
        Try
            ...
            Else
                ...
                dbAccess.updateData("UPDATE `introse`.`makeup` SET `timestart` = '" & Convert.ToDateTime(txtbxStart.Text).ToString("HH:mm:ss") & "', `timeend` = '" & Convert.ToDateTime(txtbxEnd.Text).ToString("HH:mm:ss") & "',`room` = '" & txtbxRoom.Text & "', `reason_cd` = '" & reason_cd & "', `makeup_date` = '" & dtpMakeUpDate.Value.Date.ToString("yyyy-MM-dd") & "', `enc_date` = '" & Date.Now.Date.ToString("yyyy-MM-dd") & "', `encoder` =  'unknown' WHERE `makeupid` = '" & ref & "';")
